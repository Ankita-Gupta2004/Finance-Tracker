...
export default function GoalsProgressCard() {
  ...
  useEffect(() => {
    ...
    if (savedData) {
      try {
        ...
        const allGoals = [
          ...decrypted.shortTermGoals.map((g) => ({ ...g, type: "Short-term" })),
          ...decrypted.midTermGoals.map((g) => ({ ...g, type: "Mid-term" })),
          ...decrypted.longTermGoals.map((g) => ({ ...g, type: "Long-term" })),
        ];
  ...
  return (
    <div className="h-full w-full flex flex-col">
      <h2 className="text-xl font-bold mb-2 text-gray-900 dark:text-white">Goals Progress</h2>

      {/* Legends */}
      <div className="flex items-center gap-6 mb-4 border-b border-b-gray-300 dark:border-b-gray-600 pb-2">
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-emerald-500"></span>
          <span className="text-sm text-gray-700 dark:text-gray-300">Short-term</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-blue-500"></span>
          <span className="text-sm text-gray-700 dark:text-gray-300">Mid-term</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-3 h-3 rounded-full bg-pink-500"></span>
          <span className="text-sm text-gray-700 dark:text-gray-300">Long-term</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2">
        {goalsData.length === 0 ? (
          <p className="text-gray-500 dark:text-gray-400">No goals saved yet.</p>
        ) : (
          goalsData.map((goal) => {
            ...
            return (
              <div key={goal.id} className="mb-4">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  {goal.type} | Target: ₹{goal.targetAmount || 0} | Current: ₹{goal.currentSavings || 0}
                </p>
              </div>
            );
          })
